...
struct Legend: View {
    ...
    var maxValueTextColor : Color = Color("Blue")
    var maxValueLineColor : Color = Color("DarkGrey")
    var otherValuesTextColor : Color = Color("DarkGrey")
    var otherValuesLineColor : Color = Color("DarkGrey").opacity(0.00)
            ...
            ForEach((0...2), id: \.self) { height in
                ZStack(alignment: .leading){
                    self.line(atHeight: self.getYLegendSafe(height: height), width: self.frame.width)
                        .stroke(self.colorScheme == .dark ? Colors.LegendDarkColor : (height < 2 ? otherValuesLineColor : maxValueLineColor), style: StrokeStyle(lineWidth: 1.5, lineCap: .round, dash: [5,height == 0 ? 0 : 10]))
    ...
    func getYLegend() -> [Double]? {
        ...
        return [min+step * 1, min+step * 2, min+step * 3]
    }
}
...
struct Legend_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            Legend(data: ChartData(points: [0.2,0.4,1.4,4.5], index: 0, profit: 0), frame: .constant(geometry.frame(in: .local)), hideHorizontalLines: .constant(false))
